...
ui <- fluidPage(# Application title
  ...
  sidebarLayout(
    ...
    # Show results
    mainPanel(verbatimTextOutput("vto1"))
  ))
...
server <- function(input, output) {
  output$vto1 <- renderPrint({
    CO2 |>
      ...
      ungroup() |>
      pivot_wider(id_cols = .data[[input$grpvar]],
                  values_from = avg_uptake,
...
# ---
#   CO2 |>
#   group_by(.data[["Plant"]], Treatment) |>
#   summarise(avg_uptake = mean(uptake,na.rm=T)) |>
#   ungroup() |>
#   pivot_wider(id_cols=.data[["Plant"]], values_from = avg_uptake, names_from = Treatment)
#
#
# ---
#
